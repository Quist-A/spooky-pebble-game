...
max_pebbles = 80 # maximal number of pebbles allowed
...
def gameFormulaWithoutSpooks(T, n, output_vertices, edges, max_pebbles, onePebblePerT = True):
	"""
	...
	for t in range(T-1):
		...
		for father, child in edges:
			...
			s.add(Or(Not(P[father][t]),P[father][t+1],P[child][t]))#S[father][t+1],P[child][t]))
			s.add(Or(Not(P[father][t]),P[father][t+1],P[child][t+1]))#S[father][t+1],P[child][t+1]))
